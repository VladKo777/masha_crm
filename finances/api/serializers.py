...
from finances.models import Product, Transaction
...
class TransactionSerializer(serializers.Serializer):
    ...
    currency = serializers.PrimaryKeyRelatedField(queryset=Product.objects.all())
    surcharge = serializers.DecimalField(max_digits=20, decimal_places=2)
    ...
    def create(self, validated_data, **kwargs):
        ...
        if request.GET.get('add_money', '') == 'to_storage' and account_from == account_to:
            account_from = Account.objects.filter(type=config.air_account).last()
            if not account_from:
                raise serializers.ValidationError('Встановіть в налаштуваннях поле "повітряний рахунок"!')
            is_add_money = True
        obj = Transaction.objects.create_inner_transaction(
            account_from=account_from,
            account_to=account_to,
            value=validated_data['value'],
            currency=validated_data['currency'],
            user=request.user,
            is_add_money=is_add_money
        )
        if obj.account_balance_to.account.type == config.counterparty_account:
            obj.confirm_for_counterparty(request.user)
        return obj
